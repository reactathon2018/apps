...
import { status } from "react-file-uploader";
...
class UploadDocs extends Component {
  ...
  onFileProgress(fileId, fileData) {
    const { files = [] } = this.state,
      newFiles = files.map(
        item => (item.id === fileId ? Object.assign({}, item, fileData) : item)
      );
  ...
  render() {
    return (
      <div>
        ...
        >
          <p>{!this.state.isDragOver ? "Drop here" : "Files detected"}</p>
        </Receiver>
        <div>
          <p>Upload List</p>
          <UploadManager
